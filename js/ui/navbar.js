...
$('body').on('click', '#submit-gene', function () {
    let human = dc.getOneBy('human', "id", Number($('#champcache').attr('data-id')))
    let owner = dc.userList[0]
    human.addGene(owner, $('#add-gene').val())
    if (dc.userList[0].gp < 3) {
        $('#add-gene').hide()
        $('#submit-gene').hide()
    }
})
...
$('body').on('click', '.timelapse', function () {
    ...
    switch ($(this).attr('id')) {
        case 'timelapse-play':
            world.frameDuration = 1000;
            $('#myLogs').css('transition', 'background-color 3s ease');
            break;
        case 'timelapse-play2':
            world.frameDuration = 500;
            $('#myLogs').css('transition', 'background-color 1.5s ease');
            break;
        case 'timelapse-play10':
            world.frameDuration = 100;
            $('#myLogs').css('transition', 'background-color 0.3s ease');
            break;
        case 'timelapse-play50':
            world.frameDuration = 20;
            $('#myLogs').css('transition', 'background-color 30s ease');
            break;
        case 'timelapse-pause':
            world.frameDuration = "pause";
    }
...
function getCensus() {
    ...
    if (town) {
        list = list.filter(function (ele) {
            return (ele.town == $('#chooseTown').val());
        });
    }
