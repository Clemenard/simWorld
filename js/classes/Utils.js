...
Utils.prototype.birthTraits = function (father, mother, trait) {
  ...
  for (let i = 0; i < 5; i++) {
    if (i < 2) {
      babyTalents[talentsRemaining[rand]] = father.talents[talentsRemaining[rand]];
    } else if (i < 4) {
      babyTalents[talentsRemaining[rand]] = mother.talents[talentsRemaining[rand]];
    } else {
  ...
  if (trait) {
    if (babyTalents[trait] < 8) {
      babyTalents[Number(trait)] = 10
    } else {
      babyTalents[trait] += 2;
    }
  }
...
Utils.prototype.getOwner = function (ownership) {
  ...
  ownership.forEach(element => {
    if (element && typeof (element.id) != "number" && !mainOwner[Number(element.id.id)]) {
      mainOwner[Number(element.id.id)] = 0;
    }
    if (element && typeof (element.id) != "number") {
      mainOwner[Number(element.id.id)]++;
    }
  });
  /* console.log(utils.getSortedKeys(mainOwner)) */
  if (utils.getSortedKeys(mainOwner)[0]) {
    dc.userList.forEach((user, index, childs) => {
      if (user.id == Number(utils.getSortedKeys(mainOwner)[0].key)) {
        dc.userList[index].gp += 1;
      }
    });
    return {
      id: Number(utils.getSortedKeys(mainOwner)[0].key),
      trait: utils.getSortedKeys(mainOwner)[0].value
    }
  }
  return {
    id: 0,
...
Utils.prototype.drawChart = function (data, title) {
  ...
  if (xMax < 0 || xMax > dc.age / 12) {
    xMax = Math.round(dc.age / 12);
  }
  if (xMin < 0 || xMin > dc.age / 12) {
    xMin = 0;
  }
  var options = {
    title: title,
