function Human(isGenerate = false, father, mother) {
    ...
    if (isGenerate) {
        ...
        for (let i = 0; i < 8; i++) {
            this.ownership.push({
                id: 0,
                name: dc.GENE_NAME_LIST[utils.getRandomArbitrary(0, dc.GENE_NAME_LIST.length - 1)]
            })
        }
        this.owner = {
            id: 0,
...
Human.prototype.getOlder = function () {
    ...
    if (this.age < 120) {
        ...
        learnSpeed = 0.01
    } else if (this.age < 720) {
        learnSpeed = -0.01
    } else {
        learnSpeed = -0.02
    }
    for (let i = 0; i < 6; i++) {
        this.stats[i] += Math.random() * this.talents[i] * learnSpeed
    }
}
...
Human.prototype.death = function () {
    ...
    if (world.page == this.id) {
        $('#graph-max').val(Math.floor(dc.age / 12))
    }
    this.age = -this.age;
    return true;
...
Human.prototype.getJob = function (init = true) {
    ...
    if (init) {
        ...
        job = this.JOB_LIST[utils.getRandomArbitrary(0, this.JOB_LIST.length - 1)];
    }
    if (job) {
...
Human.prototype.logDay = function (type) {
    let myself = this;
    let log = dc.alivelogList.filter(function (ele) {
        return ele.related[0] == myself.id && ele.type == type;
    })[0]
    if (log) {
...
Human.prototype.display = function (style = "basic") {
    ...
    switch (style) {
        case "history":
            html += utils.genderMark(this.sex, "name") + '<span style="opacity:0;">--</span>[' + setClasses + ' id=' + this.id + ']' + this.nobleParticle().toUpperCase() + '' + this.surname.toUpperCase() + " " + this.name + "[/id], " + this.logDay('birth') + "-" + this.logDay('death') + " (" + Math.abs(this.getAge()) + " ans)."
            break;
        case "birth":
        ...
        default:
            if (this.age < 0) {
                html += "[" + setClasses + " id=" + this.id + "]" + this.name + "[/id]";
            } else {
...
Human.prototype.getMainGenes = function () {
    let mainGenes = new Array();
    this.ownership.forEach(element => {
        if (element) {
            if (!mainGenes[element.name]) {
                mainGenes[element.name] = 0;
            }
            mainGenes[element.name]++;
        }
    });
    return mainGenes;
}
...
function Avatar(owner, name, surname, sex) {
    ...
    this.owner = owner;
    this.talents = utils.racialTraits(owner.trait);
    this.stats = jQuery.extend({}, this.talents);
    for (let j = 0; j < 6; j++) {
        this.stats[j] *= 2.5;
    }
