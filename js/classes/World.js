function World(duration = 5000000, startPop = 200, maxPop = 10000, frameDuration = 1000) {
    ...
    let avatar = new Avatar({
        id: 10,
        pseudo: dc.pseudo,
        trait: Number($('#avatar-spec').val()),
        gp: 0
    }, $('#avatar-name').val(), $('#avatar-surname').val(), $('#avatar-sex').val())
    dc.alivehumanList.push(avatar);
    let rival = new Avatar({
        id: 11,
        pseudo: "Lebogossdu49",
        trait: Number($('#rival-spec').val()),
        gp: 0
    }, 'Lara', "Croft", "female");
...
World.prototype.getTownById = function (id) {
    ...
    let search = dc.alivetownList.filter(function (ele) {
        return ele.id == id;
    })[0];
    if (search) {
        return search;
...
World.prototype.getNobleHouse = function (option) {
    let nobles = search = dc.alivehouseList.filter(function (ele) {
        return ele.state == "noble";
    });
    if (option == "random") {
...
World.prototype.getMainFamilies = function (year, town) {
    ...
    dc.census.human[year].forEach(element => {
        if (!town || dc.getOneBy("town", 'id', dc.getOneBy("house", 'id', element.house).town).id == town) {
            if (!mainFamilies[element.surname]) {
                mainFamilies[element.surname] = 0;
            }
...
World.prototype.getDataGraph = function (graphs) {
    ...
    switch (graphs) {
        case "Medium Age":
            data.push(header.concat(['Eldest']));
            break;
        case "Total Money":
            data.push(header);
            break;
        case "Total Population":
            data.push(header.concat(['0-15 yo', '15-30 yo', '30-45 yo', '45-60 yo', '60+ yo']));
            break;
    }
    dc.census.human.forEach((element, i, childs) => {
        ...
        switch (graphs) {
            case "Medium Age":
                ...
                dataRow.push(element.reduce((accumulator, currentValue) => {
                    return (accumulator > currentValue.age ? accumulator : currentValue.age);
                }, 0) / 12);
                break;
            case "Total Money":
                dataRow.push(dc.census.house[i].reduce((accumulator, currentValue) => {
                    if (currentValue.gold < 0) {
                        gold = 0
                    } else {
                        gold = currentValue.gold
                    }
                    return (accumulator + gold);
                }, 0));
                break;
            case "Total Population":
                dataRow.push(element.length);
                dataRow.push(element.filter(function (ele) {
                    return ele.age <= 180;
                }).length);
                dataRow.push(element.filter(function (ele) {
                    return (ele.age > 180 && ele.age <= 360);
                }).length);
                dataRow.push(element.filter(function (ele) {
                    return (ele.age > 360 && ele.age <= 540);
                }).length);
                dataRow.push(element.filter(function (ele) {
                    return (ele.age > 540 && ele.age <= 720);
                }).length);
                dataRow.push(element.filter(function (ele) {
                    return (ele.age > 720);
                }).length);
...
World.prototype.getHouseGraph = function (human) {
    ...
    dc.census.human.forEach((ele, i, childs) => {
        ele.forEach((e, j, childs) => {
            if (e.id == human) {
                age = i;
                gold = dc.census.house[i].filter(function (house) {
                    return (house.id == e.house);
                })[0].gold;
            }
